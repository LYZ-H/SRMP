...
class psPyTorchAdapter:
    ...
    def send_zero_grad(self, step):
        ...
        blob = _serialize_obj_to_bytes(zero_chunks)
        self._conn.data_send(blob, step)
    ...
    def update_wait(self):
        ...
        chunks =  _deserialize_bytes_to_obj(blob)
        set_param_with_chunks(self._net, chunks)
    ...
    def ps_synUpdate(self, optimizer=None):
        ...
        # update self.aggregated_param_chunks_nparray
        for blob in grads_list:
            chunks = _deserialize_bytes_to_obj(blob)
            for i, chunk in enumerate(chunks):
                n = np.frombuffer(chunk, dtype=np.float32)
                self.aggregated_param_chunks_nparray[i] += n * r

        aggregated_param_chunks = []
        for chunk in self.aggregated_param_chunks_nparray:
            aggregated_param_chunks.append(chunk.tobytes())

        blob = _serialize_obj_to_bytes(aggregated_param_chunks)
